...
def test_hybrid_rules_first(tmp_path: Path):
    # Train trivial model
    train_rows = [
    ...
    label = tag_sentence_hybrid("Assume the premise.", model=model, rules_first=True, confidence_gap=0.9)
    assert label == "M"

    # No strong rule -> ML used
    label2 = tag_sentence_hybrid("Totally ambiguous.", model=model, rules_first=True, confidence_gap=0.0)
